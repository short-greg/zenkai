...
from zenkai._core import _weight as W
...
class TestNormalizeWeight:
    ...
    def test_normalize_weight_sums_to_one(self):
        ...
        result = W.weight_normalize(weight)
    ...
    def test_normalize_weight_sums_to_one_on_dim_1(self):
        ...
        weight = torch.rand(2, 4)
        result = W.weight_normalize(weight, 1)
...
class TestRankWeight:
    ...
    def test_rank_weight_ranks_the_pop(self):
        ...
        weight = torch.tensor([[0.5, 1.0], [0.75, 0.5]])
        t = torch.tensor([[2, 1], [1, 2]], dtype=torch.float)

        ranked = W.weight_rank(weight)
    ...
    def test_rank_weight_ranks_the_pop_with_minimize(self):
        ...
        ranked = W.weight_rank(weight, maximize=True)
        assert torch.isclose(ranked, t).all()
...
class TestLogWeight:
    ...
    def test_log_weight_outputs_correct_value(self):
        ...
        weight = torch.tensor([[0.5, 1.0], [0.75, 0.5]])
        norm_weight = W.weight_normalize(weight)

        log_weight = W.weight_log(norm_weight)
...
class TestGaussCDFWeight:
    ...
    def test_log_weight_outputs_correct_value(self):
        ...
        weight = torch.tensor([[0.5, 1.0, 0.3], [0.75, 0.5, 0.4]])

        result = W.weight_gauss_cdf(weight)
