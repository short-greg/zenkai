...
class TestFA:
    ...
    def test_fa_linear_updates_the_parameters(self):
        ...
        learner = feedback_alignment.FALinearLearner(3, 4, optim_factory=OptimFactory('sgd', lr=1e-2), objective='mse')
...
class TestFALearner:
    ...
    def test_fa_learner_does_not_auto_adv_if_false(self):
        ...
        learner = feedback_alignment.FALearner(
            net, nn.Linear(3, 4), 
            optim_factory=OptimFactory('sgd', lr=1e-2), objective='mse', activation=nn.Sigmoid()
        )
        t = IO(torch.rand(3, 4))
        x = IO(torch.rand(3, 3))
        before = get_model_parameters(net)
