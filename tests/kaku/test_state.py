...
class X(core.IDable, Base):
    pass


class Y(core.IDable, nn.Linear):
...
class TestAssessmentLog:
    ...
    def test_assessment_log_update_adds(self):
        ...
        log.update(
            "x", "name", "validation", assessment
        )
        assert log.dict['x']['name']['validation'].value == assessment.value
...
class TestState:
    ...
    def test_spawn_keeps_the_sub(self):
        ...
        sub.set(obj, 'z', 2, True)
        state2 = state.spawn()
        sub2 = state2.sub(obj, 'sub2', False)
        assert sub2[obj, 'z'] == 2
