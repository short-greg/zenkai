...
class TestFactory:
    ...
    def test_factory_vars_returns_all_vars(self):
        ...
        Linear = _build.Factory(
            nn.Linear, _build.Var("in_features"), _build.Var("out_features")
        )
...
class TestBuilderArgs:
    ...
    def test_clone_clones_the_args_and_kwargs(self):
        ...
        builder_args = _build.BuilderArgs(
            args=[2, _build.Var("y")], kwargs={"x": 2, "z": _build.Var("y")}
        )
...
class SimpleLearnerBuilder(_build.Builder[SimpleLearner]):
    def __init__(self, in_features: int = None, out_features: int = None):
        ...
        self.out_features = self.Updater[SimpleLearnerBuilder, int](
            self, "out_features"
        )
...
class TestBuilder:
    def test_builder_returns_simple_learner(self):
        ...
        simple_learner = SimpleLearnerBuilder(in_features=2, out_features=3)
    ...
    def test_builder_returns_simple_learner_without_updating_the_value(self):
    ...
    def test_builder_returns_simple_learner_after_updating_value_with_updater(self):
        ...
        simple_learner_builder = SimpleLearnerBuilder(in_features=2, out_features=3)
        simple_learner = simple_learner_builder.in_features(2)()
        assert isinstance(simple_learner, SimpleLearner)

    def test_builder_returns_simple_learner_after_updating_value_with_chained_updates(
        self,
    ):
        ...
        simple_learner = simple_learner_builder(in_features=2, out_features=3)
        assert isinstance(simple_learner, SimpleLearner)
