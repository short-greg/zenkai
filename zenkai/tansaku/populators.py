...
# local
from .. import Assessment, Assessor
from .core import Individual, Population, deflatten, cat_params, binary_prob, flatten, expand
...
class StandardPopulator(Populator):
    ...
    @abstractmethod
    def populate(self, key: str, val: typing.Union[torch.Tensor, Parameter]) -> typing.Union[torch.Tensor, Parameter]:
        pass
...
class PopulatorDecorator(Populator):
    ...
    @abstractmethod
    def decorate(self, key: str, base_val, val: typing.Union[torch.Tensor, Parameter]) -> typing.Union[torch.Tensor, Parameter]:
...
class SimpleGaussianPopulator(StandardPopulator):
    ...
    def populate(self, key: str, val: typing.Union[torch.Tensor, Parameter]) -> typing.Union[torch.Tensor, Parameter]:
        ...
        perturbation = torch.randn(
            self.k - 1, *val.shape, device=val.device 
        )
        val = torch.cat([
            val[None],
            val[None] + perturbation
        ])
...
class GaussianPopulator(StandardPopulator):
    ...
    def populate(self, key: str, val: torch.Tensor):
        ...
        noise = torch.randn(
            *shape, device=val.device) * self.std
...
class BinaryPopulator(StandardPopulator):
    ...
    def __init__(
        self, k: int=1, keep_p: float=0.1, 
        equal_change_dim: int=None, to_change: typing.Union[int, float]=None, 
        reorder_params: bool=True, zero_neg: bool=False
    ):
        if 0.0 >= keep_p or 1.0 < keep_p:
            raise RuntimeError(f"Argument p must be in range (0.0, 1.0] not {keep_p}")
        assert k > 1
        self.keep_p = keep_p
        self.k = k  
        self._equal_change_dim = equal_change_dim
        self._is_percent_change = isinstance(to_change, float)
        if self._is_percent_change: assert 0 < to_change <= 1.0
        elif to_change is not None: assert to_change > 0
    ...
    def _generate_keep(self, param: torch.Tensor):
        ...
        if self._is_percent_change:
            ignore_change = (
                torch.rand(1, 1, *param.shape[1:], device=param.device) > self._to_change
            ).type_as(param)
        else:
            _, indices = torch.rand(math.prod(param.shape[1:]), device=param.device).topk(self._to_change, dim=-1)
    ...
    def spawn(self) -> 'BinaryPopulator':
        return BinaryPopulator(
            self.k, self.keep_p, self._equal_change_dim, self._to_change, 
            self._reorder_params, self._zero_neg
        ) 
...
class ConservativePopulator(PopulatorDecorator):
    ...
    def __init__(
        self, base_populator: Populator, percent_change: float=0.1,
        same_change: bool=True
    ):
        super().__init__(base_populator)
        if not (0.0 <= percent_change <= 1.0):
            raise ValueError(f"Percent change must be between 0 and 1")
        self.percent_change = percent_change
        self.same_change = same_change

    def decorate(self, key: str, base_val: torch.Tensor, val: typing.Union[torch.Tensor, Parameter]) -> typing.Union[torch.Tensor, Parameter]:
...
class PerceptronProbPopulator(Populator):
    ...
    def __init__(self, learner: Assessor, k: int, x: str="x", unactivated: str="unactivated"):
        self.k = k
    ...
    def populate(self, key: str, val: typing.Union[torch.Tensor, Parameter]) -> typing.Union[torch.Tensor, Parameter]:
        ...
        return (x_[None] < torch.rand(self.k, *x_.shape, device=x_.device)).type_as(x_) * 2 - 1


class BinaryProbPopulator(Populator):
    
    def __init__(self, learner: Assessor, k: int, zero_neg: bool=True, loss_name: str='loss', x: str="x", t: str="t"):
        self.learner = learner
        self.k = k
    ...
    def generate_sample(self, base_size: torch.Size, dtype: torch.dtype, device=torch.device, prob: torch.Tensor=None):
...
class PopulationLimiter(object):
    ...
    def __call__(self, individual: Individual, population: Population, limit: torch.LongTensor=None) -> Population:
