...
# Local
from ._convert import freshen


class IO(tuple):
    """A container for wrapping inputs and outputs to a learning machine.

    It provides extra functionality to the tuple class to interact with the
    tensors it wraps.
    """
    ...
    def detach(self) -> Self:
        ...
        return IO(x.detach() if isinstance(x, torch.Tensor) else x for x in self)
    ...
    def apply(self, f: typing.Callable):
        ...
        """
        return IO(f(x_i) for x_i in self)
    ...
    def dx(self, x_prime: typing.Iterable) -> "IO":
        ...
        """
        return IO(val - x_prime[i] if i < len(x_prime) else None for i, val in enumerate(self))

    def on(self, module: nn.Module) -> "IO":

        return IO([module(*[x for x in self])])
    ...
    def tensor_only(self) -> "IO":
        ...
        """
        return IO(x_i if isinstance(x_i, torch.Tensor) else None for x_i in self)
    ...
    def acc_grad(self, lr: typing.Optional[float] = 1.0) -> "IO":
        ...
        Returns:
            IO: The IO with the updated x
        """
        return IO(self._acc_grad(x, lr) for x in self)
    ...
    def acc_dx(self, dx: typing.Union["IO", typing.Iterable], lr: typing.Optional[float] = 1.0) -> "IO":
        ...
        Returns:
            IO: The IO with the updated x
        """
        return IO(self._acc_dx(x, dx_i, lr) for x, dx_i in zip(self, dx))
    ...
    def acc_t(self, t: typing.Union["IO", typing.Iterable], lr: float = 1.0) -> "IO":
        ...
        """
        return IO(
            ((1 - lr) * x) + (lr * t_i) if isinstance(x, torch.Tensor) and t_i is not None else x
    ...
    def grad(self) -> "IO":
        ...
        Returns:
            IO: The IO with the updated x
        """
        return IO(x.grad if isinstance(x, torch.Tensor) else x for x in self)
    ...
    def t(self, dy: typing.Iterable) -> Self:
        ...
        """
        return IO(
            val - dy[i] if i < len(dy) and isinstance(dy[i], torch.Tensor) else None for i, val in enumerate(self)
        )
    ...
    def to_x(self) -> typing.Union[typing.Tuple, typing.Any]:
...
def merge_io(x: typing.List[IO], f: typing.Callable = None) -> IO:
    ...
    return IO(f(*x_i) if f is not None else x_i for x_i in zip(*x))


def pipe(
    modules: typing.Iterable[nn.Module],
    x: torch.Tensor,
    freshen_h: bool = False,
    get_h: bool = False,
) -> typing.Union[torch.Tensor, typing.Tuple[torch.Tensor, IO]]:
    ...
    Returns:
        typing.Union[torch.Tensor, typing.Tuple[torch.Tensor, IO]]: The output of the
        last module and optionally the intermediate values
    """
    hs = []
    for m in modules:
        x = m(x)
...
def minibatch_io(
    xs: typing.Union[typing.Iterable[IO], IO],
    shuffle: bool = False,
    batch_size: int = 1,
    drop_last: bool = False,
) -> typing.Iterator[typing.Tuple[torch.Tensor]]:
    ...
    for xs in torch_data.DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, drop_last=drop_last):
