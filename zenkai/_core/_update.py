...
def update_mix(cur_val: torch.Tensor, prev_val: torch.Tensor, exp_p: float = 1.0) -> torch.Tensor:
...
def update_mean(cur: torch.Tensor, mean: torch.Tensor = None, dim: int = -1, weight: float = 0.9) -> torch.Tensor:
    ...
    return (1 - weight) * cur_mean + weight * mean
...
def update_var(
    ...
    return (1 - weight) * cur_var + weight * var
...
def slope_calc(val: torch.Tensor, assessment: torch.Tensor) -> torch.Tensor:
    """Calculate the slope based on the assessment

    Args:
        val (torch.Tensor): The value
        assessment (torch.Tensor): The assessment for the value. Must have fewer or the same number of
            dimensions as val but not be 0-dim

    Returns:
        torch.Tensor: The slope
    """
    evaluation = tensor_align(assessment, val)
    base_shape = val.shape
    val = val.reshape(val.size(0), val.size(1), -1)
    ssx = (val**2).sum(0) - (1 / len(val)) * (val.sum(0)) ** 2
    ssy = (val * evaluation).sum(0) - (1 / len(val)) * (val.sum(0) * evaluation.sum(0))
    slope = ssy / ssx
    return slope.reshape(base_shape)


def scale_calc(cur_val: torch.Tensor, ref: torch.Tensor, scale: float = None) -> torch.Tensor:
