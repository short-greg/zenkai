...
def sample_gaussian(mean: torch.Tensor, std: torch.Tensor, k: int = None) -> torch.Tensor:
...
def noise_gaussian(x: torch.Tensor, std: float = 1.0, mean: float = 0.0) -> torch.Tensor:
...
def noise_binary(x: torch.Tensor, flip_p: bool = 0.5, signed_neg: bool = True) -> torch.Tensor:
