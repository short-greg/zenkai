...
def minibatch_loop(*x: torch.Tensor, shuffle: bool = True, drop_last: bool = False) -> typing.Iterator[torch.Tensor]:
    """
    ...
    Args:
        *x (torch.Tensor): One or more tensors to be split into minibatches.
        ...
        drop_last (bool, optional): If True, drops the last incomplete minibatch if the dataset size is not
            divisible by the batch size. Defaults to False.
    Returns:
...
def module_filter(module: nn.Module, filter_type: typing.Type):
    """
    ...
    for child in module.children():
        if isinstance(child, filter_type):
            yield child
        yield from module_filter(child, filter_type)


def module_apply(module: nn.Module, apply_type: typing.Type, f: typing.Callable):
    """
    ...
    for child in module.children():
        if isinstance(child, apply_type):
            f(child)
        module_apply(child, apply_type, f)
