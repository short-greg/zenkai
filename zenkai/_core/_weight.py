...
# local
from ._shape import unsqueeze_vector


def weight_normalize(weight: torch.Tensor, pop_dim: int = 0) -> torch.Tensor:
...
def weight_softmax(weight: torch.Tensor, pop_dim: int = 0, maximize: bool = False) -> torch.Tensor:
    ...
    """
    if maximize:
        return torch.softmax(weight, dim=pop_dim)
    return torch.nn.functional.softmin(weight, dim=pop_dim)


def weight_rank(weight: torch.Tensor, pop_dim: int = 0, maximize: bool = False) -> torch.Tensor:
    ...
    """
    _, ind = weight.sort(dim=pop_dim, descending=not maximize)

    # Create the ranks
    ranks = torch.arange(1, weight.shape[pop_dim] + 1, 1, device=weight.device)
...
def weight_log(norm_weight: torch.Tensor, maximize: bool = False, eps: float = 1e-7) -> torch.Tensor:
...
def weight_gauss_cdf(weight: torch.Tensor, pop_dim: int = 0) -> torch.Tensor:
