...
from ._lm import LearningMachine, acc_dep, forward_dep
...
class GlobalTargetLearner(LearningMachine):
    ...
    def forward_iter(self, x: IO, state: State, **kwargs) -> typing.Iterator[typing.Tuple[LearningMachine, IO, State]]:
        ...
        Returns:
            typing.Iterator[typing.Tuple[LearningMachine, IO, State]]: The machine, the input and the
                state from each submachine
        """
        pass

    @abstractmethod
    def optim_x(self, x: IO, t: IO, state: State) -> IO:
        """Determine the value of ``x`` to propagate backwards.
    ...
    def forward_nn(self, x: IO, state: State, **kwargs) -> typing.Union[typing.Tuple, typing.Any]:
        """Process the input through each submachine and return the final output.

        This method iterates through the submachines, processes the input, and stores the
        intermediate inputs and sub states on the state.
        ...
        Returns:
            typing.Union[typing.Tuple, typing.Any]: The final output after processing through all submachines.
        """
        xs = []
        sub_states = []
        y = None
    ...
    def accumulate(self, x: IO, t: IO, state, **kwargs):
        """Accumulate parameter updates for each submachine.

        This method gets the target for each submachine using the ``optim_x`` method
        ...
        for (machine, t, _), x, sub_state in zip(self.forward_iter(x_new, state, **kwargs), xs, sub_states):
    ...
    def step_x(self, x, t, state, **kwargs):
        """Return the new ``x`` computed during accumulation.
