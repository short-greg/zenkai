...
def step_dep(check_field: str):
    ...
    def inner(func):
        ...
        def _(self: "LearningMachine", x: IO, t: IO, state: State, *args, **kwargs):
            ...
            val = state.get(check_field)
            if val is None:
                raise RuntimeError(f"Method {func} for {type(self)} depends on step() but step() has not been executed")
            return func(self, x, t, state, *args, **kwargs)
...
def to_grad(flattened_dx: typing.List) -> typing.List:
    """Convert a flattened list of dx values to a tuple of gradients.

    Args:
        flattened_dx (typing.List): The flattened dx values

    Returns:
        typing.List: The gradients, with non-tensors converted to None
    """
    return tuple(dx if isinstance(dx, torch.Tensor) else None for dx in flattened_dx)
...
class _OutT(torch.autograd.Function):
    ...
    def backward(ctx, *grad_output):
        ...
        x = ctx.saved_tensors
        module = ctx.module
        module.t = IO(x_i - g_i for x_i, g_i in zip(x, grad_output))
    ...
    for k, v in storage.items():
        ...
        if k == "__sub__":
            for k2, v2 in v.items():
                sub = state.sub(k2)
                _load_state_helper(sub, v2, t)

        elif isinstance(v, IO):
            state[k] = IO(t[v_i.idx] if isinstance(v_i, TId) else v_i for v_i in v)
...
def out(x, multi: bool = True) -> typing.Union[typing.Any, typing.Tuple]:
    """Helper function to output the value

    Args:
        x: The value to output
        multi (bool, optional): Whether there are multiple outputs. Defaults to True.

    Returns:
        typing.Union[typing.Any, typing.Tuple]: The detached output
    """
    if multi:
        return tuple(x_i.detach() if isinstance(x_i, torch.Tensor) else x_i for x_i in x)
...
class StepXHook(ABC):
    ...
    @abstractmethod
    def __call__(self, step_x: "StepX", x: IO, y: IO, x_prime: IO, t: IO, state: State) -> typing.Tuple[IO, IO]:
        pass
...
class StepHook(ABC):
    ...
    @abstractmethod
    def __call__(self, step: "StepTheta", x: IO, y: IO, t: IO, state: State) -> typing.Tuple[IO, IO]:
        pass
...
class LearnerPostHook(ABC):
    ...
    @abstractmethod
    def __call__(self, x: IO, y: IO, t: IO, assessment: torch.Tensor) -> typing.Tuple[IO, IO]:
        pass
...
class LearningMachine(nn.Module, ABC):
    ...
    def forward_hook(self, hook: ForwardHook, consummable: bool = False) -> "LearningMachine":
        ...
        Args:
            hook (ForwardHook): The hook to add
        """
...
class OutDepStepTheta(StepTheta):
    ...
    @abstractmethod
    def step(self, x: IO, y: IO, t: IO, outgoing_t: IO = None, outgoing_x: IO = None):
...
class InDepStepX(StepX):
    ...
    @abstractmethod
    def step_x(self, x: IO, y: IO, t: IO, incoming_x: IO = None, incoming_t: IO = None) -> IO:
...
def backward(lm: LearningMachine, x: IO, t: IO, state: State, lmode: LMode = None) -> IO:
    """
    ...
    Args:
        ...
        lmode (LMode, optional): The lmode, if it is None it will use the lmode on the learning machine.
            Defaults to None.
    ...
    if lmode == LMode.OnlyStepX:
        ...
        x_prime = lm.step_x(x, t, state)
        return x_prime
    raise RuntimeError(f"Unknown mode {lm.lmode}")
