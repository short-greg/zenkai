...
# local
from zenkai._core import IO, Reduction
from zenkai.nnz._assess import Criterion

from ._objective import Constraint, Objective, impose
...
class FuncObjective(Objective):
    ...
    def forward(self, reduction: str, **kwargs: torch.Tensor) -> torch.Tensor:
        ...
        constraints = self._constraint(**kwargs)
        mask = None
        for cur in constraints.values():
            mask = cur if mask is None else (mask | cur)
        value = impose(value, mask, self._penalty)
