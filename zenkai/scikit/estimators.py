...
from .. import utils
from ..kaku import (
    IO, Conn, FeatureLimitGen, FeatureIdxStepX, FeatureIdxStepTheta, Idx,
    FeatureLimitGen, AssessmentDict, LearningMachine, State, Loss
)
...
class ScikitEstimator(nn.Module):
    ...
    def __init__(
        self, sklearn_estimator, in_features: int=1, 
        out_features: int=1, partial_fit: bool=True, 
        use_predict: bool=True, backup: nn.Module=None
    ):
        """initializer
        ...
        if partial_fit and not hasattr(self._sklearn_estimator, 'partial_fit'):
            raise RuntimeError("Using partial fit but estimator does not have partial fit method available")
        
        if not partial_fit and not hasattr(self._sklearn_estimator, 'fit'):
            raise RuntimeError("Using fit but estimator does not have fit method available")
        ...
        self._is_multioutput = isinstance(sklearn_estimator, MultiOutputClassifier) or isinstance(sklearn_estimator, MultiOutputRegressor)
    ...
    def _prepare(self, y: np.ndarray, limit: typing.List[int]) -> typing.List[BaseEstimator]:
        """
        ...
        if not (isinstance(self._sklearn_estimator, MultiOutputClassifier) or isinstance(self._sklearn_estimator, MultiOutputRegressor)):
            return y, None
    ...
    def _replace_estimators(self, cur_estimators: typing.List[BaseEstimator], limit: typing.List[int]):
    ...
    def _partial_fit(self, X: torch.Tensor, y: torch.Tensor, limit: typing.List[int]=None):
        if limit is not None and not self.fitted:
            raise RuntimeError(f"Must fit model before setting a limit")
        X = utils.to_np(X)
        y = utils.to_np(y)
    ...
    def _full_fit(self, X: torch.Tensor, y: torch.Tensor, limit: typing.List[int]=None):
...
class ScikitRegressor(ScikitEstimator):
    ...
    def __init__(
        self, sklearn_estimator, in_features: int=1, 
        out_features: int=1, multi: bool=False, partial_fit: bool=True, 
        ...
        super().__init__(
            sklearn_estimator, in_features, out_features, partial_fit, 
            use_predict, backup
        )
...
class ScikitMulticlass(ScikitEstimator):
    ...
    def __init__(
        self, sklearn_estimator, in_features: int, n_classes: int,
        multi: bool=False, 
        out_features: int=1, partial_fit: bool=True, 
        use_predict: bool=True,
        ...
        super().__init__(
            sklearn_estimator, in_features, out_features, 
            partial_fit, use_predict, backup
        )
...
class ScikitMachine(LearningMachine, FeatureIdxStepX, FeatureIdxStepTheta):
    ...
    def __init__(
        self, module: ScikitEstimator, step_x: FeatureIdxStepX, loss: Loss,
