...
class DXTargetPropLearner(LearningMachine):
    ...
    def step(self, x: IO, t: IO, state: State):
        ...
        assessment = self.forward_machine.assess(y, IO(x[1], detach=True), detach=False)
        assessment['loss'].backward()
        self.optim.step()
    ...
    def forward(self, x: IO, state: State, detach: bool = True) -> IO:
        ...
        y = state[self, 'y'] = IO(x[0].detach() + dy)
        return y.out(detach)


class AEDXTargetPropLearner(LearningMachine):

    def __init__(self, net: TargetPropNet, forward_net: nn.Module, loss: TargetPropLoss, ae_loss: Loss, optim_factory: OptimFactory, assessment_name: str='loss'):
        """initializer
