# local
from ..kaku.machine import Conn, State
from ..tansaku.populators import GaussianPopulator, BinaryPopulator, PopulationLimiter
from ..tansaku.selectors import BestSelectorFeature, SlopeSelector
...
from ..kaku.machine import (
    LearningMachine, State,
    State, Conn, IO, FeatureIdxStepX, FeatureIdxStepTheta, Idx, update_io
)


class HillClimbStepX(FeatureIdxStepX):

    def __init__(self, learner: LearningMachine, k: int, std: float=0.1, lr: float=1e-2, momentum: float=0.5, maximize: bool=False):
    ...
    def step_x(self, conn: Conn, state: State, feature_idx: Idx=None) -> Conn:
        ...
        population = self.limiter(
            individual, self.populator(individual), 
