...
from ..kaku.machine import (
    State, IO, Conn, LearningMachine, 
    ThLoss, StepX, StepTheta, AssessmentDict,
    update_io
)
...
class LeastSquaresStandardSolver(LeastSquaresSolver):
    ...
    def solve(self, a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
        ...
        """        
        new_, _, _, _ = scipy.linalg.lstsq(
            *self._prepare(to_np(a), to_np(b))
        )
...
class LeastSquaresStepTheta(StepTheta):
    ...
    def __init__(
        ...
        optimize_dw: bool=False, lr: typing.Optional[float]=None
    ): 
        """initializer
    ...
    def _optimize_dw(self, x: torch.Tensor, t: torch.Tensor):
        ...
        dweight, dbias = self.split(self.solver.solve(
            x, t_delta
        ))
    ...
    def _optimize_w(self, x: torch.Tensor, t: torch.Tensor):
        weight, bias = self.split(self.solver.solve(
            x, t
        ))
...
class LeastSquaresStepX(StepX):
    ...
    def __init__(
        self, linear: nn.Linear, 
        solver: LeastSquaresSolver=LeastSquaresStandardSolver, 
        optimize_dx: bool=False, lr: typing.Optional[float]=None
    ): 
        """initializer
    ...
    def _optimize_dx(self, x: torch.Tensor, t: torch.Tensor):
        ...
        t_delta = t - y
        dx = self.solver.solve(
            self.linear.weight, t_delta.T
        )
        return x + dx
...
class LeastSquaresLearner(LearningMachine):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool=True, optimize_dx: bool=True, x_lr: float=1e-2, x_reduction: str="mean"):
        super().__init__()
        self._linear = nn.Linear(in_features, out_features, bias)
        self._loss = ThLoss("mse", 'mean')
        self._step_x = LeastSquaresStepX(self._linear, LeastSquaresRidgeSolver(1e-4, False), optimize_dx)
        self._step_theta = LeastSquaresStepTheta(self._linear, LeastSquaresRidgeSolver(1e-3, bias=bias), optimize_dx)
